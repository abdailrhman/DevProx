...
|-------------------------------------------|
|This Source Was Developed By (AEN) @H6HHHH.|
|  This Is The Source Channel @witblk .   |
|               - DevProx -                 |
---------------------------------------------
]]..'\27[m')
sudos = dofile("./config.lua")
DevId = sudos.SUDO
SudoUsers = {sudos.sudo_users,1330377150}
DevProx = sudos.bot_id
TokenBot = sudos.token
NameBot = (DevAbs:get(DevProx..'Abs:NameBot') or 'وايت بلاك')
...
if text and text:match('^'..(DevAbs:get(DevProx..'Abs:NameBot') or "وايت بلاك")..' ') then
data.message_.content_.text_ = data.message_.content_.text_:gsub('^'..(DevAbs:get(DevProx..'Abs:NameBot') or "وايت بلاك")..' ','')
end
...
if Sudo(msg) then 
local Sudo_Welcome = '⋄︙اهلا بك مجددا عزيزي المطور \n⋄︙اليك الازرار الخاصه بتعديل وتغيير كلايش سورس ديف وايت بلاك فقط اضغط على الامر الذي تريد تنفيذه'
local key = {
...
Dev_Abs(msg.chat_id_, msg.id_, 1, "⋄︙عذرا عزيزي ↫ ["..data.first_name_.."](t.me/witblk)\n⋄︙الصوره التي ارسلتها تم منعها من المجموعه", 1, 'md')
end
end,nil)   
DeleteMessage(msg.chat_id_,{[0] = msg.id_})       
return false   
...
Dev_Abs(msg.chat_id_,0, 1, "⋄︙عذرا عزيزي ↫ ["..data.first_name_.."](t.me/witblk)\n⋄︙الملصق الذي ارسلته تم منعه من المجموعه", 1, 'md')
end
end,nil)   
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
return false   
...
local BotName = (DevAbs:get(DevProx.."Abs:NameBot") or 'وايت بلاك')
...
if text and text == (DevAbs:get(DevProx..'Abs:NameBot') or 'وايت بلاك') then 
NameBot = (DevAbs:get(DevProx..'Abs:NameBot') or 'وايت بلاك')
...
⋄︙[𝘪𝘯𝘧𝘰𝘳𝘮𝘢𝘵𝘪𝘰𝘯](http://t.me/Fileswitblk)
    ┈┈┈┈┈┈┈┈┈┈┈┈┈┈
⋄︙[𝘥𝘦𝘷𝘦𝘭𝘰𝘱𝘦𝘳](http://t.me/hppppph)
    ┈┈┈┈┈┈┈┈┈┈┈┈┈┈
⋄︙[𝘵𝘸𝘴 𝘵𝘦𝘢𝘮](http://t.me/hppppph)
└───────CR───────┘
]]
Dev_Abs(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
...
if tonumber(result.id_) == tonumber(1330377150) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, 'دي لكك تريد اهينن تاج راسكك؟😏🖕🏿', 1, 'md') 
return false  
end  
...
if LinkGp.ok == true then LinkGroup = LinkGp.result else LinkGroup = 't.me/witblk' end
...
Dev_Abs(msg.chat_id_, msg.id_, 1, '⋄︙جاري تحديث سورس ديف وايت بلاك', 1, 'md') 
...
DevAbs:set(DevProx..'Abs:NameBot',(DevAbs:get('ABS_PROX:'..DevProx..'name_bot') or 'وايت بلاك'))
end
for i = 1, #Users do
local id = Users[i]
if id:match("^(%d+)") then
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/abdailrhman/WITBLKFiles/master/getfile.json")
if res == 200 then
...
local GetJson, Res = https.request("https://raw.githubusercontent.com/abdailrhman/WITBLKFiles/master/DevProxFiles/"..FileName)
if Res == 200 then
